...
class MyStorage {
    ...
    removeItem(id) {
        const index = this.items.findIndex((i) => JSON.stringify(i) === JSON.stringify(id));
        if (index === -1)
            return `${id} not found in storage.`;
        const removed = this.items.splice(index, 1)[0];
        return `${removed.name || removed} removed from storage.`;
    }
    findItem(prop, val) {
        return this.items.find((i) => typeof i === "object" && i !== null && i[prop] === val);
    }
    updateItem(prop, id, update) {
        const index = this.items.findIndex((i) => typeof i === "object" && i !== null && i[prop] === id);
        if (index === -1)
            return `Item not found.`;
        const oldItem = this.items[index];
        this.items[index] = update;
        return `${oldItem.name || oldItem} updated successfully.`;
    }
}
// Test cases
